...
# Load the data with all columns as characters
data <- read_csv("models.csv", col_types = cols(.default = "c"))

# Ensure all columns are character vectors and handle NA values
data <- data %>%
  mutate(across(everything(), ~replace_na(as.character(.), "")))

# View the first few rows of the data for debugging
head(data)

# Convert the R dataframe to a pandas dataframe in Python
...
data = pd.read_csv("models.csv", dtype=str)  # Ensure all columns are read as strings
